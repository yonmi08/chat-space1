    ...
    context 'log in' do
      ...
      it 'assigns @message' do
        expect(assigns(:message)).to be_a_new(Message)
         # be_a_newマッチャの利用で、対象が引数で指定したクラスのインスタンスかつ未保存のデコードであるかどうか確かめることができる
      ...
      it 'renders index' do
        expect(response).to render_template :index # expectの引数にresponseを渡している。
      end
    end
    ...
    # この中にログインしている場合のテストを記述
      ...
      # この中にメッセージの保存に成功した場合のテストを記述
        ...
        it 'count up message' do
          ...
          # Messageモデルのレコードの総数が1個増えたかどうかを確かめることができる。
          # →保存に成功すると、レコードの数は1個増えるため
        end
      ...
      context 'can not save' do # ログインしている、かつ保存に失敗した場合
